# Sponsorship Tiers

## Overview

Financial sponsorship helps sustain SmartContractAudit development, infrastructure, and community. We offer multiple tiers to accommodate different levels of support.

## Important Notes

- **Independence**: Sponsorship does NOT grant control over project decisions
- **Transparency**: All sponsors publicly listed
- **Community First**: Open source community remains our priority
- **Tax**: We are not currently a registered non-profit; consult your tax advisor

See [GOVERNANCE.md](/GOVERNANCE.md) for sponsor influence policy.

## Sponsorship Tiers

### 🥉 Bronze Sponsor

**Monthly**: $100 | **Annual**: $1,000 (save $200)

**Recognition:**
- Name in README sponsors section
- Name in website footer
- Sponsor badge on GitHub profile
- Thank you in release notes

**Benefits:**
- Community Slack/Discord access
- Sponsor-only updates newsletter
- Vote in sponsor polls (non-binding input)

**Ideal For:**
- Startups using the project
- Individual developers
- Small teams
- Community supporters

---

### 🥈 Silver Sponsor

**Monthly**: $500 | **Annual**: $5,000 (save $1,000)

**Recognition:**
- Small logo in README
- Small logo on website sponsors page
- Mention in quarterly blog posts
- Social media recognition

**Benefits:**
- All Bronze benefits
- Priority community support
- Quarterly roadmap preview
- 1 sponsored feature request per year (community approval required)
- Listed in conference/talk acknowledgments

**Ideal For:**
- Growing blockchain projects
- Development agencies
- Mid-size companies
- Active ecosystem participants

---

### 🥇 Gold Sponsor

**Monthly**: $2,000 | **Annual**: $20,000 (save $4,000)

**Recognition:**
- Medium logo in README (prominent placement)
- Medium logo on website homepage
- Dedicated sponsor spotlight blog post
- Logo in presentation materials
- Press release mention

**Benefits:**
- All Silver benefits
- Dedicated support channel
- Monthly sync with maintainers
- 3 sponsored feature requests per year
- Early access to beta features (1 week before public)
- Co-marketing opportunities
- Conference booth collaboration
- Custom integration support (up to 20 hours/year)

**Ideal For:**
- Established blockchain projects
- Audit firms
- Infrastructure providers
- Companies with significant stake in smart contract security

---

### 💎 Platinum Sponsor

**Annual**: $50,000+

**Recognition:**
- Large logo in README (top placement)
- Large logo on website homepage hero
- Dedicated case study/success story
- Logo on all marketing materials
- Named sponsor for major features/releases
- Press releases and announcements

**Benefits:**
- All Gold benefits
- Weekly technical liaison
- 10 sponsored feature requests per year
- Early access to all features (2 weeks before public)
- Custom deployment assistance
- Private security vulnerability disclosure (before public if you're affected)
- Priority bug fixes for your use cases
- Annual on-site training (travel expenses separate)
- Dedicated Slack channel with core team
- Custom SLA available

**Ideal For:**
- Major blockchain platforms
- Enterprise users
- Foundational infrastructure
- Organizations with critical security needs

---

### 🎯 Custom Sponsorship

**Contact for Pricing**

For organizations with unique needs:
- Custom recognition packages
- Specific feature development
- Enterprise deployment support
- Training and consulting
- Research collaborations

**Contact**: funding@cuberai.example

## Additional Sponsorship Options

### One-Time Donation

**Any Amount**: One-time contribution

**Recognition:**
- Thank you in next release notes
- Name in contributors list
- Warm fuzzy feeling

**Ideal For:**
- Individuals
- One-time supporters
- Project beneficiaries wanting to give back

---

### In-Kind Sponsorship

**Value Varies**

**Examples:**
- Infrastructure credits (AWS, GCP, Azure)
- CI/CD service credits
- Development tools licenses
- Conference booth space
- Marketing services
- Legal services

**Recognition**: Based on equivalent monetary value

---

### Community Sponsorship

**No Cost**

**Examples:**
- Technical contributions (code, docs, reviews)
- Community moderation and support
- Content creation (tutorials, videos)
- Event organization
- Translation and internationalization

**Recognition**: Contributor status, mentions in relevant contexts

## What Sponsorship Funds

Your sponsorship supports:

### Development (50%)
- Core maintainer time
- Feature development
- Bug fixes and maintenance
- Security audits
- Code reviews

### Infrastructure (25%)
- GitHub organization fees
- CI/CD compute resources
- Cloud hosting
- Domain and SSL certificates
- Development tools and services

### Community (15%)
- Documentation
- Tutorial creation
- Community management
- Support channels
- Event participation

### Operations (10%)
- Legal and accounting
- Marketing and outreach
- Swag and recognition
- Emergency fund

## Sponsorship Terms

### Duration
- **Monthly**: Cancel anytime with 30 days notice
- **Annual**: 12-month commitment, auto-renews unless canceled 60 days before renewal

### Recognition
- Added within 5 business days of first payment
- Remains for duration of sponsorship plus 30 days
- Removed upon cancellation or non-payment

### Feature Requests
- Subject to community approval
- Must align with project goals
- No guarantee of implementation
- Timeline based on maintainer capacity
- Publicly tracked like all features

### Support
- Response time targets (not guarantees unless custom SLA)
- Bronze: 5 business days
- Silver: 3 business days
- Gold: 1 business day
- Platinum: 4 hours (business hours)

### Changes
- Tier benefits may change with 60 days notice
- Price increases not applied to existing sponsors for 12 months
- New sponsors get current pricing and benefits

## How to Sponsor

### GitHub Sponsors (Preferred)

1. Visit our GitHub Sponsors page: [github.com/sponsors/SolanaRemix](https://github.com/sponsors/SolanaRemix)
2. Select your tier
3. Complete payment setup
4. Automatic recognition in README

### Open Collective

1. Visit our Open Collective page: [opencollective.com/smartcontractaudit](#)
2. Choose tier or custom amount
3. Complete payment
4. We'll contact you for logo/recognition details

### Direct Payment

For Platinum or Custom sponsorships:

1. Email funding@cuberai.example
2. We'll send invoice
3. Payment via bank transfer, check, or other method
4. Formal agreement if needed

### Cryptocurrency

We accept:
- Bitcoin (BTC)
- Ethereum (ETH)
- Solana (SOL)
- USDC/USDT stablecoins

Contact funding@cuberai.example for wallet addresses.

## Frequently Asked Questions

**Q: Can we sponsor anonymously?**  
A: Yes, for Bronze and Silver. Gold+ requires public recognition for transparency.

**Q: Do you offer discounts for non-profits?**  
A: Yes, 50% discount for registered non-profits and educational institutions. Contact us for details.

**Q: Can we sponsor a specific feature?**  
A: Yes! Custom sponsorship. Feature must have community approval.

**Q: What if we can't afford sponsorship but want to help?**  
A: Community sponsorship (contributions) is equally valued! Code, docs, and support are invaluable.

**Q: Can we cancel anytime?**  
A: Monthly sponsors can cancel anytime. Annual sponsors committed for the year but won't auto-renew if canceled.

**Q: Do you provide invoices?**  
A: Yes, automatic for GitHub Sponsors and Open Collective. Manual invoices for direct payments.

**Q: What currency do you accept?**  
A: USD primary. Can accommodate EUR, GBP, and cryptocurrency.

## Recognition Examples

### README Example

```markdown
## Sponsors

### 💎 Platinum
[<img src="platinum-logo.png" width="300">](https://platinum-sponsor.example)

### 🥇 Gold
[<img src="gold1-logo.png" width="200">](https://gold1.example)
[<img src="gold2-logo.png" width="200">](https://gold2.example)

### 🥈 Silver
[<img src="silver-logo.png" width="150">](https://silver.example)

### 🥉 Bronze
- Bronze Sponsor 1
- Bronze Sponsor 2
```

## Contact

**General Inquiries**: funding@cuberai.example  
**Custom Sponsorship**: funding@cuberai.example  
**Technical Questions**: GitHub Issues  
**Existing Sponsors**: Your dedicated channel or funding@cuberai.example

---

**Last Updated**: 2026-01-01  
**Current Sponsors**: 0 (new program)  
**Accepting Applications**: Yes
